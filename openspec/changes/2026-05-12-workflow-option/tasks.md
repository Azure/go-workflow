...
- [ ] 1.1 Create `workflow_option.go` with `WorkflowOption` struct (pointer scalars, slice fields, `DontInherit bool`)
- [ ] 1.2 Add `WorkflowOptionReceiver` interface with `InheritOption(parent WorkflowOption)`
- [ ] 1.3 Implement `prependSlice[T any](parent, child []T) []T` helper that always allocates a fresh backing array (godoc: "MUST NOT mutate either input")
...
- [ ] 3.1 Add `findOptionReceiver(s Steper) WorkflowOptionReceiver` mirroring the removed `findInterceptorReceiver`
- [ ] 3.2 In `Workflow.Do()`, after `init()` and before `tick`, walk root steps and call `recv.InheritOption(w.Option)` once per receiver
- [ ] 3.3 In `Workflow.Do()`, snapshot `w.Option` immediately after acquiring `isRunning` lock and `defer` restore
- [ ] 3.4 Remove the Mutator dispatch site that called `recv.PrependMutators` (currently around `workflow.go:641`)
- [ ] 3.5 Remove the Interceptor dispatch site that called `findInterceptorReceiver` / `recv.PrependInterceptors` (currently around `workflow.go:768`)
- [ ] 3.6 Simplify `effectiveStepInterceptors` / `effectiveAttemptInterceptors` to return `w.Option.StepInterceptors` / `w.Option.AttemptInterceptors` directly (parent contributions are already merged in by `InheritOption`)

## 4. Removals

- [ ] 4.1 Delete `MutatorReceiver` interface from `mutator.go`
- [ ] 4.2 Delete `InterceptorReceiver` interface and `findInterceptorReceiver` from `interceptor.go`
- [ ] 4.3 Delete `(*Workflow).PrependMutators` method
- [ ] 4.4 Delete `(*Workflow).PrependInterceptors` method
- [ ] 4.5 Delete `(*SubWorkflow).PrependMutators` and `(*SubWorkflow).PrependInterceptors` methods
...
- [ ] 6.1 Apply MODIFIED Requirements from `changes/2026-05-12-workflow-option/specs/workflow-options/spec.md` to `openspec/specs/workflow-options/spec.md`
- [ ] 6.2 Apply MODIFIED Requirements from `changes/2026-05-12-workflow-option/specs/composite-steps/spec.md` to `openspec/specs/composite-steps/spec.md`:
...
- [ ] 6.3 Apply MODIFIED Requirements from `changes/2026-05-12-workflow-option/specs/mutators/spec.md` to `openspec/specs/mutators/spec.md`:
...
- [ ] 6.4 Apply MODIFIED Requirements from `changes/2026-05-12-workflow-option/specs/step-interceptor/spec.md` to `openspec/specs/step-interceptor/spec.md`:
...
- [ ] 7.1 Migrate `workflow_options_test.go`: every `&Workflow{ Field: ... }` literal rewritten to `Option: WorkflowOption{ ... }`; scalars wrapped as pointers; assertions unchanged
- [ ] 7.2 Migrate `workflow_mutator_test.go`: `Workflow.Mutators` references → `Option.Mutators`; tests of `PrependMutators` rewritten to exercise `InheritOption`
- [ ] 7.3 Migrate any other test file that references the nine former fields or the removed receiver interfaces
- [ ] 7.4 Create `workflow_option_inherit_test.go` with table-driven matrix:
...
- [ ] 7.7 Test: `SubWorkflow` deprecation smoke — embedded `SubWorkflow` still receives parent's Option via delegated `InheritOption`
- [ ] 7.8 Test: scalar inheritance covers the motivating use case — parent with `DontPanic=true`, child without setting it; verify child's executed code path also recovers panics
- [ ] 7.9 Test: `Workflow{}` zero value (no Option set) runs identically to before for a single-workflow case (no panic, no overhead)

## 8. Examples

- [ ] 8.1 Update `example/13_mutators_test.go` example that references `flow.SubWorkflow` to embed `flow.Workflow` directly and use `Option`
- [ ] 8.2 Update any other example file referencing the nine former fields or `flow.SubWorkflow`
- [ ] 8.3 Add example demonstrating scalar inheritance: parent sets `DontPanic`, child inherits it without restating

## 9. Documentation

- [ ] 9.1 Update `Workflow` godoc: remove field-by-field documentation of the nine former fields; add reference to `Option` and `WorkflowOption`
...
- [ ] 10.1 `go build ./...` — no compile errors
- [ ] 10.2 `go test ./...` — all migrated and new tests pass
- [ ] 10.3 `go vet ./...` — no issues
- [ ] 10.4 `grep -rn "PrependMutators\|PrependInterceptors\|MutatorReceiver\|InterceptorReceiver\|IsolateInterceptors\|inheritedStep\|inheritedAttempt\|findInterceptorReceiver" --include="*.go"` returns only expected residual hits (none in source; possibly historical refs in archived openspec)
- [ ] 10.5 `grep -rn "DefaultOption\|w\.MaxConcurrency\|w\.DontPanic\|w\.SkipAsError\|w\.Clock\|w\.Mutators\|w\.StepInterceptors\|w\.AttemptInterceptors" --include="*.go" | grep -v _test.go` — no source-side direct reads of the removed fields
- [ ] 10.6 Confirm `SubWorkflow`-embedding example compiles, runs, and inherits parent Option correctly
