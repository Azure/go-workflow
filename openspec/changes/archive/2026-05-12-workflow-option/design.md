...
| `StepBuilder` (type) fate | Mark deprecated at the type level now; remove next major | The `BuildStep` *method* was already deprecated by the Mutator change. The type itself is embedded by `Workflow` and therefore promoted onto any user struct embedding `flow.Workflow` — same surface-area concern this change is solving for the nine option fields. Behavior is unchanged this release; type + method + user hook + implicit `Workflow.Add` invocation all remain until the next major. |
